...
describe('risk fallback paths', () => {
  it('returns a deterministic result when stale cache is the only available BCRA source', () => {
    ...
    expect(result.discountRate).toBe(0.126);
  });

  it('never crashes when BCRA data is missing and falls back to a deterministic worst-case baseline', () => {
    const result = scoreRiskDeterministically({
      source: 'fallback',
      asOfDate: '2026-03-28',
      dueDate: '2026-04-15',
    ...
    expect(result.signals).toContain('Fallback determinístico por ausencia de datos BCRA en vivo');
  });
});
